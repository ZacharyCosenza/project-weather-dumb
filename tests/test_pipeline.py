...
@pytest.fixture(scope="session")
def model_temp(hourly):
    """Train on the real hourly data. Fast XGB params; splits within the window."""
...
def test_data_science(model_temp):
    assert model_temp.n_features_in_ > 0


def test_inference(hourly, model_temp):
    result = run_inference(hourly, model_temp, _FEATURE_COLS)

    assert {"timestamp", "features", "temp"} <= result.keys()
    assert isinstance(result["temp"]["prediction_c"], float)
    assert isinstance(result["temp"]["prediction_f"], float)
    assert set(result["temp"]["shap"].keys()) == set(_FEATURE_COLS)
...
def test_predictions_schema():
    ...
    assert "prediction_c" in p["temp"]
    assert "prediction_f" in p["temp"]
    assert "shap"         in p["temp"]
